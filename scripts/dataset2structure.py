    ...
    for sent in doc.sentences:
        ...
        for word in sent.words:
            ...
            # else:
                ...
                dependency_index.add_term(word.text, sentence_id= counter)
                if word.head > 0 :
                    parent =  sent.words[word.head - 1].text
                    # dependency_index.add_term(parent, sentence_id= counter)
                    dependency_index.add_dependency(parent, (word.text, word.deprel))
        

        counter +=1

dependency_index.write_index_to_file('index_children.json', format='json')

with open('headwords.txt', 'r') as file:
    # Read all lines and strip newline characters
    wor = [line.strip() for line in file]
...
common = []
for word in wor:
    if dependency_index.search_term(word):
        common.append(word)
